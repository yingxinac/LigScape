################################################################################
# Score each ligand in LigScape for input data.
# Input:
# siggroups: list of signatureid-vectors for each ligand in LigScape.
...
# pvalcut: p-value cutoff, only signatures with score-pvalues below this cutoff are used for scoring the ligands in LigScape.
# Output:
# A data.frame of ligand scores and the signature sources.
# For each ligand, each column scores the ligand by the set of signatures of it in LigScape (with the score-pvalues <= specified pvalcut):
...
# Remark: for ligands in LigScape with no signature score-pvalue <= pvalcut, the the first seven columns are set to -2, and the other six columns are set to "none";
...
# for ligands that are not included in LigScape, all columns are set to NA.
################################################################################

#' Score the ligands in LigScape
#'
#' @param siggroups list of signatureid-vectors for each ligand in LigScape.
...
#' @param pvalcut p-value cutoff, only signatures with score-pvalues below this cutoff are used for scoring the ligands in LigScape.
#'
#' @return
#' a data.frame of ligand scores and the signature sources
#' @import dplyr Seurat stringr RColorBrewer pheatmap ggplot2 fgsea graphite lsa devtools data.table geneconverter circlize tidyverse
#' @export
